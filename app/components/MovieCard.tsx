...
const MovieCard = ({ id, title, poster, year, rating = 0, voteCount = 0, overview = '' }: MovieCardProps) => {
  return (
    <Link href={`/movie/${id}`} className="group block">
      <div className="relative w-full aspect-[2/3] overflow-hidden rounded-2xl transition-all duration-500 group-hover:scale-105 group-hover:shadow-card-hover animate-scale-in">
        {/* Glow effect */}
        <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-primary-500/20 via-transparent to-accent-500/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-xl"></div>
        
        {/* Main card */}
        <div className="relative h-full glass border border-white/10 rounded-2xl overflow-hidden">
          {poster ? (
            <>
              <Image
                src={poster}
              ...
              <div className="absolute inset-0 p-5 flex flex-col justify-end transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                ...
                <div className="space-y-2">
                  <h3 className="font-bold text-lg text-white leading-tight line-clamp-2 group-hover:text-primary-300 transition-colors duration-300">{title}</h3>
                ...
                <div className="mt-4 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-4 group-hover:translate-y-0">
                  <div className="inline-flex items-center gap-2 glass-dark px-4 py-2 rounded-full text-sm font-semibold text-white hover:bg-primary-500/20 transition-colors duration-300">
