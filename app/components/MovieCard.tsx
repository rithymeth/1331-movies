  ...
  return (
    <Link href={`/movie/${id}`} className="group">
      <div className="relative overflow-hidden rounded-lg bg-gray-800/50 backdrop-blur-sm transition-all duration-300 group-hover:scale-105 group-hover:bg-gray-800/80 border border-gray-700/50 group-hover:border-blue-500/50 shadow-lg hover:shadow-blue-500/20">
        ...
        <div className="p-4">
          <h3 className="text-lg font-semibold line-clamp-2 group-hover:text-blue-400 transition-colors duration-300">{title}</h3>
          <div className="flex items-center justify-between mt-2">
            <p className="text-sm text-gray-400">{year}</p>
            {voteCount > 0 && (
              <p className="text-xs text-gray-500">{voteCount.toLocaleString()} votes</p>
            )}
          </div>
        </div>
      </div>
