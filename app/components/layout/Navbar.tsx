      ...
      <div className="container mx-auto px-6">
        <div className="flex items-center justify-between py-4">
          ...
          <div className="flex items-center space-x-3">
            <button type="button" onClick={() => window.dispatchEvent(new Event('open-command-search'))} className="hidden xl:inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs font-semibold text-slate-300 hover:bg-white/10" aria-label="Open search palette">
              Search
            ...
            <form onSubmit={handleSearch} className="hidden md:flex items-center relative group" ref={searchRef}>
              <div className="relative">
                <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search titles..." className="input-modern w-56 pl-12 pr-4 py-3 text-white placeholder-white/60 lg:w-64" />
            ...
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="lg:hidden glass p-3 rounded-xl text-white/90" aria-label="Toggle menu">
        ...
        <div className={`lg:hidden overflow-hidden transition-all duration-500 ${isMenuOpen ? 'max-h-[32rem] opacity-100' : 'max-h-0 opacity-0'}`}>
