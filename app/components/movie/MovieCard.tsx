...
const MovieCard = ({ id, title, poster, year, rating = 0, voteCount = 0, overview = '' }: MovieCardProps) => {
  return (
    <Link href={`/movie/${id}`} className="group block">
      <div className="relative w-full aspect-[2/3] overflow-hidden rounded-xl transition-all duration-500 group-hover:-translate-y-1 group-hover:shadow-2xl animate-scale-in">
        {/* Glow effect */}
        ...
        {/* Main card */}
        <div className="relative h-full overflow-hidden rounded-xl border border-white/10 bg-[#11161d]">
          {poster ? (
            <>
              <Image
                src={poster}
              ...
              <div className="absolute inset-0 p-5 flex flex-col justify-end transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                {/* Rating badge */}
                {rating > 0 && (
                  <div className="absolute top-3 right-3 rounded-md border border-white/15 bg-black/70 px-2 py-1 flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-4 group-hover:translate-x-0">
                ...
                <div className="mt-4 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-4 group-hover:translate-y-0">
                  <div className="inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-semibold text-slate-950 transition-all duration-300">
