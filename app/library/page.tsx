  ...
  return (
    <main className="min-h-screen bg-[#080b10] px-4 pb-16 pt-28 sm:px-6">
      <div className="mx-auto max-w-6xl">
          ...
          <section className="mb-10">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {history.slice(0, 3).map((entry) => {
                const href = entry.type === 'movie' ? `/movie/${entry.id}` : `/tv-shows/${entry.id}`;
                return (
        ...
        <header className="mb-8 flex flex-col gap-5 border-b border-white/10 pb-7 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <input
              ...
              className="rounded-md border border-white/10 bg-white/5 px-3 py-2 text-sm text-white outline-none placeholder:text-slate-500 focus:border-cyan-300/50"
            />
            <button type="button" onClick={shareLibrary} disabled={items.length === 0} className="inline-flex items-center gap-2 rounded-md border border-white/10 bg-white/5 px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/10 disabled:opacity-40">
              <ArrowUpTrayIcon className="h-4 w-4" /> Share
            </button>
            <button type="button" onClick={clearLibrary} disabled={items.length === 0 && history.length === 0} className="rounded-md border border-red-400/20 bg-red-400/5 px-3 py-2 text-sm font-medium text-red-200 hover:bg-red-400/10 disabled:opacity-40">
              Clear all
            </button>
            {(['all', 'movie', 'tv'] as Filter[]).map((value) => (
              <button
                ...
                className={`rounded-md border px-3 py-2 text-sm font-medium ${
                  filter === value ? 'border-cyan-300 bg-cyan-300 text-slate-950' : 'border-white/10 bg-white/5 text-gray-300 hover:bg-white/10'
                }`}
              >
        ...
        {shareMessage && (
          <p role="status" className="mb-6 rounded-md border border-cyan-300/20 bg-cyan-300/10 px-4 py-3 text-sm text-cyan-200">{shareMessage}</p>
        )}
        {visibleItems.length === 0 ? (
          <section className="rounded-xl border border-dashed border-white/15 bg-white/[0.03] px-6 py-16 text-center">
            ...
            <h2 className="mt-4 text-xl font-semibold text-white">{items.length === 0 ? 'Your library is waiting' : 'Nothing in this filter yet'}</h2>
            <p className="mx-auto mt-2 max-w-md text-sm text-gray-400">
              {items.length === 0 ? 'Use Save on any movie or show page and it will appear here.' : 'Try another filter or browse the catalog for something new.'}
            </p>
            <Link href="/movies" className="mt-6 inline-flex rounded-md bg-cyan-300 px-4 py-2 text-sm font-semibold text-slate-950 hover:bg-cyan-200">Browse movies</Link>
          </section>
        ) : (
              ...
              return (
                <article key={`${item.type}-${item.id}`} className="group">
                  <div className="relative aspect-[2/3] overflow-hidden rounded-xl border border-white/10 bg-[#121923]">
                    {item.posterPath ? (
                      <Image src={`https://image.tmdb.org/t/p/w500${item.posterPath}`} alt={item.title} fill sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 20vw" className="object-cover transition duration-500 group-hover:scale-105" />
                    ) : (
                    ...
                    <div className="absolute inset-x-0 bottom-0 flex justify-between gap-2 bg-gradient-to-t from-black/90 to-transparent p-3 pt-10">
                      <Link href={href} className="inline-flex items-center gap-1.5 rounded-md bg-white px-3 py-2 text-xs font-semibold text-slate-950 hover:bg-cyan-200">
                        <PlayIcon className="h-3.5 w-3.5" /> Open
                      </Link>
