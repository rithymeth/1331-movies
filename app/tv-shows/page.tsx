  ...
  return (
    <div className="min-h-screen animated-bg">
      ...
      <div className="relative z-10 mx-auto max-w-7xl space-y-8 px-4 py-10 sm:px-8">
        ...
        {filteredView ? (
          <>
            <MediaGrid items={filtered} emptyTitle="No series match these filters" emptyMessage="Try another year, country, language, sort, or genre." />
            <CatalogPager basePath="/tv-shows" query={query} page={filteredPage.page} totalPages={filteredPage.totalPages} />
          </>
        ) : (
          <>
            <section><h2 className="mb-6 text-3xl font-bold text-white">Popular</h2><MediaGrid items={lists?.popular || []} emptyTitle="No popular series available" emptyMessage="Check back soon for the latest TV picks." /></section>
            <section><h2 className="mb-6 text-3xl font-bold text-white">Top rated</h2><MediaGrid items={lists?.topRated || []} emptyTitle="No top rated series available" emptyMessage="Try again later for refreshed TV rankings." /></section>
            <section><h2 className="mb-6 text-3xl font-bold text-white">Airing today</h2><MediaGrid items={lists?.airingToday || []} emptyTitle="No series airing today" emptyMessage="Airing episodes will show up here when the feed refreshes." /></section>
            <section><h2 className="mb-6 text-3xl font-bold text-white">On the air</h2><MediaGrid items={lists?.onTheAir || []} emptyTitle="No series currently on the air" emptyMessage="Currently airing shows will appear here when TMDB data is available." /></section>
          </>
        )}
      </div>
    </div>
